...
class youka {
    ...
    directGetSessionid (qq) {
        ...
        return new Promise((resolve, reject) => {
            superagent.get(data_url).set(this.config.browserMsg).redirects(1).end((err, res) => {
                //获取cookie
                if (typeof res.headers["set-cookie"] == 'object' && res.headers["set-cookie"].length > 1) {
                    let cookie = [];

                    cookie.push(res.headers["set-cookie"][0]);
        ...
        return this.directGetSessionid(qq)
            ...
            .catch(err => {
                return this.getSignCookie()
                    .then(cookies => {
                        return this.getJavascriptCookie(cookies);
                    })
                    .then(cookies => {
                        this.getSessionid(cookies);
                    })
                    .catch(err => {
                        return err;
                    });
